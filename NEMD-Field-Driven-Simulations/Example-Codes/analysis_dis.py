...
# Center_of_Reference = 0 for System Center of Mass (CoM), 1 for Polymer Center of Mass (PoM), 2 for Solvent Center of Mass (CoM), 3 for Center of Volume (CoV), 4 if System Center of Mass data was saved using Sys_CoM.py
...
Size_arr = np.array([('M', 0.00), ('H', 1.20), ('C',1.70), ('O',1.52), ('N',1.55), ('S',1.80), ('P',1.80), ('LI', 1.82), ('NA', 2.27), ('K', 2.75), ('MG', 1.73), ('F', 1.47), ('CL', 1.85), ('BR', 1.47), ('I', 1.98)])
...
# NOTE: Must supply with unwrapped coordinates
    ...
    for frame in frame_ids:
        ...
        if Center_of_Reference == '0':
            ...
            CoM = uta.select_atoms("resname SOL").center_of_mass()
        elif Center_of_Reference == '3':
            CoM = np.sum(np.array(uta.select_atoms("all").positions * (sys_radii[:, np.newaxis]**3)), axis=0)/np.sum(sys_radii**3)
