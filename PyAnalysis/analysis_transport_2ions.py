...
script, trj_file, top_file, cation_name, anion_name, t_min, t_max, step, nt, CoM_check = argv
...
# Example: python3 ${Path}/analysis_transport_2ions.py unwrap.trr md.tpr -1 -1 -1 128 0
...
def define_atom_types(uta, cation_type = 'name '+cation_name, anion_type = 'name '+anion_name):
    """ 
...
def create_position_arrays(uta, cations, anions, frame_ids):
    """
...
def compute_all_Lij(cation_positions, anion_positions, times):
    ...
    msds_all = [msd_cation, msd_self_cation, msd_anion, msd_self_anion, msd_cross_catan]
    return msds_all
...
def main(trj_file, top_file, t_min, t_max, step, nt):
    ...
    cation_positions, anion_positions = create_position_arrays(uta, cations, anions, frame_ids)
    ...
    z_a1 = 1; z_a2 = -1
    v_a1 = 1; v_a2 = 1
    N_a1 = len(cations); N_a2 = len(anions)
    N_a1a2 = (-(z_a1*N_a1 * z_a2*N_a2) / (z_a1*z_a1*N_a1 + z_a2*z_a2*N_a2)) * ((v_a1 + v_a2) / (v_a1 * v_a2))

    msds = np.array([msds_all[1]/N_a1, msds_all[3]/N_a2, (msds_all[0] - msds_all[1])/N_a1, (msds_all[2] - msds_all[3])/N_a2, msds_all[0]/N_a1, msds_all[2]/N_a2, msds_all[4]/N_a1a2])
    
    outPrint('msd_self_'+cation_name, times, msds[0])
    outPrint('msd_self_'+anion_name,  times, msds[1])

    outPrint('msd_dis_'+cation_name, times, msds[2])
    outPrint('msd_dis_'+anion_name,  times, msds[3])

    #outPrint('msd_all_'+cation_name, times, msds[4])
    #outPrint('msd_all_'+anion_name,  times, msds[5])

    outPrint('msd_cross_'+cation_name+'_'+anion_name, times, msds[6])
