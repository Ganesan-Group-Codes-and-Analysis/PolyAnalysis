...
# Example of Use: python3 analysis_PSD.py md.xtc md.tpr "moltype MOL" "resname SOL or name LI or name CL" 1.4 90000 100000 80 80
#                 Polymer chains are molecules with name MOL -> this defines the "system"
#                 Water and salt molecules define the number of atoms within the free volume
...
def iDist(frame):
    ...
    while count < N_spheres:
        sphere_temp = np.round(rng.random((N_probe,3))*cell[:3], decimals=5); count += N_probe              # sphere_temp is a reduced-size array containing voxel-center coordinates
        ...
        remaining = np.where(radii_check == False)[0]                                                           # Index of sphere centers that still need their size determined
        while len(remaining) > 0:
            d += 2

            pair_arr, dist_arr = distances.capped_distance(sphere_temp[remaining], sys, d, box=cell)        # Distance between sphere centers and system atoms
            ...
            remaining = np.where(radii_check == False)[0]                                                          # Sphere is within the van der waals surface of the system
        del remaining
    ...
    d_arr = np.arange(0, 50.0 + 0.10, 0.10); PSD_arr = np.zeros_like(d_arr, dtype=int)                                                    # d_arr is the histogram of free volume sphere sizes; PSD_arr tracks the number of instances of points contained within free volume spheres of size at least d
