"""
Copyright (C) 2020-2024 Nico Marioni <nmarioni@utexas.edu>, Everett Zofchak <eszofchak@utexas.edu>
...
# Calculates the ion pair autcorrelation funtions (continuous, st, and/or dicontinuous, ct)
...
# Example: python3 ${Path}/analysis_st.py md.trr md.tpr NA CL 3.67 75000 100000 1 10000 128



def stct_calc1(num_s, num_c, num_count, df_max, df):
# Calculates both the continuous and discontinuous autocorrelation function
#
# Inputs: num_s = array containing st data; num_c = array containing ct data; num_count = array counting frames analyzed
#         df_max = max frame step for calculations; df = current frame step
        ...
        for i in range(df+1, min(df+1+df_max, len(dset2))):
                ...
                num_s[i-df] += np.sum(arr_s); num_c[i-df] += np.sum(arr_c); num_count[i-df] += 1
...
# Calculates the discontinuous autocorrelation function
#
# Inputs: num_c = array containing ct data; num_count = array counting frames analyzed
#         df_max = max frame step for calculations; df = current frame step
        if df%1000 == 0:
                print("dFrame "+ str(df))
        ...
        for i in range(df+1, min(df+1+df_max, len(dset2))):
                ...
                num_c[i-df] += np.sum(arr_c); num_count[i-df] += 1
...
# Calculates the continuous autocorrelation function
#
# Inputs: num_s = array containing st data; num_count = array counting frames analyzed
#         df_max = max frame step for calculations; df = current frame step
        if df%1000 == 0:
                print("dFrame "+ str(df))
        ...
        for i in range(df+1, min(df+1+df_max, len(dset2))):
                ...
                num_s[i-df] += np.sum(arr_s); num_count[i-df] += 1
...
def main(trj_file, top_file, a1_name, a2_name, coord, t_min, t_max, step, t_calc, nt):
    ...
    # Calculates both the continuous and discontinuous autocorrelation function
    num_s, num_c, num_count = np.zeros(int(dt_max / dt)+1,dtype=np.uintc), np.zeros(int(dt_max / dt)+1,dtype=np.uintc), np.zeros(int(dt_max / dt)+1,dtype=np.uintc)
    ...
    with open('st_{}_{}.xvg'.format(a1_name, a2_name), 'w') as anaout:
        print('# Time st(t)', file=anaout)
        for i in range(len(st)):
            print('{:10.3f} {:10.5f}'.format(i*dt, st[i]), file=anaout)

    with open('ct_{}_{}.xvg'.format(a1_name, a2_name), 'w') as anaout:
    ...
    # Calculates the discontinuous autocorrelation function
    #num_c, num_count = np.zeros(int(dt_max / dt)+1,dtype=np.uintc), np.zeros(int(dt_max / dt)+1,dtype=np.uintc)
    #pool = mp.Pool(processes=nt)
    #resevoir = [ti for ti in range(0,len(frame_ids),5)]
    ...
    # Calculates the continuous autocorrelation function
    #num_s, num_count = np.zeros(int(dt_max / dt)+1,dtype=np.uintc), np.zeros(int(dt_max / dt)+1,dtype=np.uintc)
    #pool = mp.Pool(processes=nt)
    #resevoir = [ti for ti in range(0,len(frame_ids),5)]
