...
# Example of Use: python3 analysis_PSD.py md.xtc md.tpr "moltype MOL" 1.4 90000 100000 80 80
...
def iDist(frame):
    ...
    sphere_arr = np.zeros((len(vox_x)*len(vox_y)*len(vox_z), 3)); radii_arr = np.zeros((len(vox_x)*len(vox_y)*len(vox_z))); count = 0
    for count_i, x in enumerate(vox_x):
        count_i += 1
        for y in vox_y:
            for z in vox_z:
                sphere_arr[count] = np.round([x,y,z], decimals = 5); count += 1
        # int(A/len(vox_y)/len(vox_z)) -> A is the max number of points to analyze at a time, adjust as needed based on memory
        if (x != vox_x[-1]) and (count_i % int(1500000/len(vox_y)/len(vox_z)) != 0):
            continue

        sphere_arr = np.round(sphere_arr, decimals = 5)
        ...
        remaining = np.where(radii_arr[:count] == 0)[0]                                                     # Index of voxel-centers that still need their size determined
        while len(remaining) > 0:
            d += 2
    
            pair_arr, dist_arr = distances.capped_distance(sphere_arr[remaining], sys, d, box=cell)         # Distance between voxel-centers and system atoms
            ...
            # Fill radii_arr for all voxel-centers that contain system atoms within d Angstroms
            ...
            if skip == 0:
                ...
                if r_min > 0:
                    radii_arr[remaining[c_index]] = r_min                                                   # Minimum distance between voxel-center and system atom
                else:
                    radii_arr[remaining[c_index]] = -1                                                      # Sphere does not overlap the system
            del pair_arr; del dist_arr
    
            remaining = np.where(radii_arr[:count] == 0)[0]                                                 # Sphere is within the van der waals surface of the system
        del remaining
    ...
    FFV_probe = []                                                                                                                  # voxel-centers to analyze
    ...
    for count_i, x in enumerate(vox_x):
        ...
        FFV_check = np.zeros((len(FFV_probe)), dtype=bool)                                                                          # FFV_check == False if this voxel-center has not been evaluated yet
        ...
        while d < max_radius:
            ...
            if r_OLD > 0 and d > 2*probe_radius and (r_OLD - len(remaining))/len(remaining) < 0.10:
                d = max_radius + 0.5
    ...
    d_arr = np.arange(0, 10.10, 0.10); PSD_arr = np.zeros_like(d_arr, dtype=int)                                                    # d_arr is the histogram of free volume sphere sizes; PSD_arr tracks the number of instances of voxels contained within free volume spheres of size at least d

    PSD_probe = np.array(FFV_save); PSD_temp = []
        ...
        for i,c in enumerate(pair_arr[:,0]):
            if c > c_index:
                ...
                if i == len(pair_arr[:,0]) - 1:                                                                                     # Check to make sure the last voxel-center is counted
                    skip = 1
        if skip == 0:
            contained = pair_arr[index:i][np.where(dist_arr[index:] < 0)[0],1]                                                               # Index of all free volume spheres containing the voxel-center
            if len(contained) > 0:                                                                                                  # If at least 1 free volume sphere contains the voxel-center, find the largest sphere and add it to PSD_arr in a cumulative manner
                max_size = 2 * np.max(radii_arr[contained])
                PSD_arr[np.where(d_arr <= max_size)[0]] += 1
        del pair_arr; del dist_arr
